        ...
        <Card>
          ...
          <div className="mt-5 space-y-3">
            {metrics.attentionPayments.map(payment => (
              <div key={payment.id} className="rounded-2xl bg-white/[0.035] p-4">
                <div className="flex items-center justify-between gap-3"><div><p className="font-bold">{payment.customer?.name}</p><p className="mt-1 text-sm text-slate-400">{payment.order?.title}</p></div><Badge>{payment.status}</Badge></div>
                <p className="mt-3 text-sm font-black text-sky-200">Restante: {formatCurrency(payment.remainingAmount)}</p>
              </div>
            ))}
          </div>
        </Card>
